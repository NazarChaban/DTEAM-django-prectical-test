...
def send_cv_pdf_email_task(self, cv_id, recipient_email, pdf_content):
    ...
    try:
        ...
        print(f"Preparing to email PDF for CV: {cv_name} (ID: {cv_id}) to {recipient_email}")

        if not pdf_content:
            error_message = f"No PDF content provided for CV ID {cv_id}. Email not sent."
            print(error_message)
        ...
        filename = f"{cv_instance.firstname}_{cv_instance.lastname}_CV.pdf".replace(" ", "_")
        email.attach(filename, pdf_content, 'application/pdf')
        email.send()

        print(f"Email sent successfully to {recipient_email} for CV: {cv_name}")
        return f"Email for CV '{cv_name}' (ID: {cv_id}) sent to {recipient_email}"

    except CV.DoesNotExist:
        print(f"CV with ID {cv_id} not found.")
        return f"Failed to send email: CV with ID {cv_id} not found."
    except Exception as e:
        print(f"Error in send_cv_pdf_email_task for CV ID {cv_id} to {recipient_email}: {e}")
